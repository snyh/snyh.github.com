module Jekyll
  ...
  class ArchiveIndex < Page
    def initialize(site, base, dir, type)
      ...
      self.process(@name)
      self.read_yaml(File.join(base, '_layouts'), type + '.tpl')
      self.data['collated_posts'] = self.collate(site)

      blogdump, year, month, day = dir.split('/')
  ...
  class ArchiveGenerator < Generator
    ...
    def generate(site)
      ...
      self.collated_posts.keys.each do |y|
        write_archive_index(site, "blog/%04d" % y.to_s, 'archive_yearly') if site.layouts.key? 'archive_yearly'
        self.collated_posts[ y ].keys.each do |m|
          write_archive_index(site, "blog/%04d/%02d" % [ y.to_s, m.to_s ], 'archive_monthly') if site.layouts.key? 'archive_monthly'
        end
      end
    end
